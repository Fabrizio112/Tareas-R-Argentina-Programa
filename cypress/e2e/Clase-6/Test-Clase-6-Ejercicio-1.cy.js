describe('Test de la Clase 6 del Ejercicio 1', () => {
  describe('Basic Working Test', () => {
    beforeEach('Visitar la pagina ', () => {
      cy.visit('http://127.0.0.1:8080/Clase-6/Ejercicio1/')
    })
    it('Test if the first part of the porgram is working', () => {
      cy.get('#cantidad-familia').clear()
    ...
    it('Test if the second part of the porgram is working', () => {
      ...
      expect(cy.get('#calcular-edad').should('not.visible'))
      it('Test if the container results is writing good the results', () => {
        expect(cy.get('#resultado #mayor').should('have.text', 'La mayor edad de todas es 2'))
        expect(cy.get('#resultado #menor').should('have.text', 'La menor edad de todas es 2'))
        expect(cy.get('#resultado #promedio').should('have.text', 'El promedio de  edad de todo el grupo familiar es 2'))
      })
    })
    it('Test if the button "Empezar de nuevo" is working', () => {
      ...
      cy.get('#calcular-familia').click();
      cy.get('#reset').click();
      expect(cy.get('#entrada-personas').should("not.visible"));
      expect(cy.get('#botones').should("not.visible"));
